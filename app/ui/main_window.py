    ...
    def seed_demo_cases(
        ...
        *,
        ...
        for tx in itertools.chain.from_iterable(scenarios):
            ...
            domain = (
                evaluated[0].indicator.domain.name
                if evaluated
                else "UNKNOWN"
            )
            alert = Alert(
            ...
            policy_result = policy_engine.evaluate_case({"band": None}, [alert_row])
            case = Case(
                id=case_id,
                alerts=[alert],
                ...
                policy_triggers=policy_result.triggered_policies,
                policy_explanations=policy_result.explanations,
                policy_flag=bool(policy_result.triggered_policies),
            )
            self.db.record_case(case)
            ...
            self.db.set_case_policy(
                case_id,
                policy_result.band,
                policy_result.triggered_policies,
                policy_result.explanations,
            )
            created += 1
            if created >= minimum:
                break
