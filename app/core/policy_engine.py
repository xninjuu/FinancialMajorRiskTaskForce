...
class PolicyEngine:
    ...
    def evaluate_case(self, case_row: Any, alerts: List[Any]) -> PolicyResult:
        alerts_data = [self._coerce_alert_dict(a) for a in alerts]
    ...
    def _build_metrics(self, case_row: Any, alerts: List[dict]) -> Dict[str, Any]:
        ...
        band = None
        if hasattr(case_row, "get"):
            band = case_row.get("band")
        elif hasattr(case_row, "band"):
            band = getattr(case_row, "band")
        elif hasattr(case_row, "keys") and "band" in case_row.keys():
            band = case_row["band"]
        return {
    ...
    def _coerce_alert_dict(self, alert: Any) -> Dict[str, Any]:
        ...
        if hasattr(alert, "__dict__"):
            ...
            return {
                "id": data.get("id"),
                "risk_level": data.get("risk_level") or data.get("status") or data.get("priority"),
        ...
        try:
            ...
            return {"id": None, "risk_level": None, "score": None, "domain": None}
